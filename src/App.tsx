...
export default function App() {
  const [listaURL, setListaURL] = useState<string>("");
  const [arquivo, setArquivo] = useState<File | null>(null);
  const [itens, setItens] = useState<Item[]>([]);
  const [log, setLog] = useState<string[]>([]);
  const [planilhaID, setPlanilhaID] = useState<string>("");
  const [abaNome, setAbaNome] = useState<string>("Produtos");
  const [agendar, setAgendar] = useState<boolean>(false);
    ...
    setTimeout(() => {
      if (listaURL.trim()) {
        ...
        setTimeout(() => {
          ...
          setItens(MOCK_ITEMS);
          addLog(`3 produtos extraídos e normalizados.`);
          if (planilhaID) {
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-2xl shadow p-4">
            ...
            <input
              ...
              value={listaURL}
              onChange={(e) => setListaURL(e.target.value)}
            />
            <p className="text-xs text-gray-600">
              Suporta páginas de resultado ou coleção com paginação ou rolagem infinita.
            </p>
          </div>
        ...
        <section className="bg-white rounded-2xl shadow p-4 mb-6">
          ...
          {itens.length === 0 ? (
            <p className="text-sm text-gray-600">
              Sem dados ainda. Cole um link ou envie um arquivo e clique em
              <span className="font-medium"> Rodar processamento</span>.
            </p>
          ) : (
            <div className="grid md:grid-cols-3 gap-3">
              {itens.slice(0, 5).map((it) => (
                <div key={it.id} className="border rounded-xl p-3 flex gap-3">
                  <img
                    src={it.imagem}
                    alt={it.produto}
                  ...
                  <div className="text-sm">
                    <div className="font-medium line-clamp-2">{it.produto}</div>
                    <div className="mt-1">
                      R$ {it.preco_atual.toFixed(2).replace(".", ",")}{" "}
                      {it.preco_anterior ? (
                        <span className="text-gray-400 line-through ml-1">
                          R$ {it.preco_anterior.toFixed(2).replace(".", ",")}
                        </span>
                      ) : null}
                    </div>
                    {it.desconto_percentual > 0 && (
                      <div className="text-green-600">-{it.desconto_percentual}%</div>
                    )}
                    <a
